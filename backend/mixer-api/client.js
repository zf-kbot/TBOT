...
const twitcherbotRequest = require("request").defaults({
    'User-Agent': `Twitchbot/${app.getVersion()}`,
    'Client-ID': CLIENT_ID
});

require('request-debug')(twitcherbotRequest, function (type, data) {
...
const twitcherbotRequestRunner = {
    run: (options) => {
        return new Promise((resolve, reject) => {
            twitcherbotRequest(options, (error, response) => {
                if (error) {
                    reject(error);
                    return;
                }
...
const streamerClient = new Mixer.Client(twitcherbotRequestRunner);
