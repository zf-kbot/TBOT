...
async function connectToMixplay() {
    ...
    try {
        ...
        eventManager.triggerEvent("twitcherbot", "mixplay-connected", {
            username: "Twitchbot"
        });
...
async function updateParticipantWithUserData(twitcherbotUser, participant = null) {
    let updateObj = {};

    let hours = twitcherbotUser.minutesInChannel < 60 ? 0 : Math.floor(twitcherbotUser.minutesInChannel / 60);
    updateObj.viewTime = `${util.commafy(hours)} hrs`;

    updateObj.mixplayInteractions = util.commafy(twitcherbotUser.mixplayInteractions);
    updateObj.chatMessages = util.commafy(twitcherbotUser.chatMessages);

    if (twitcherbotUser.currency) {
        let currencyIds = Object.keys(twitcherbotUser.currency);
        for (let currencyId of currencyIds) {
            updateObj[`currency:${currencyId}`] = util.commafy(twitcherbotUser.currency[currencyId]);
        }
    }

    await updateParticipantWithData(twitcherbotUser._id, updateObj, participant);
}
...
mixplayClient.state.on('participantJoin', async participant => {
    ...
    if (!participant.anonymous) {
        ...
        let twitcherbotUser = await userDatabase.getUserById(participant.userID);
        if (twitcherbotUser != null) {
            await updateParticipantWithUserData(twitcherbotUser, participant);
        }
...
ipcMain.on("controlUpdated", function (_, id) {
    if (!mixplayIsConnected()) return;
    let twitcherbotControl = controlManager.getControlById(id, mixplayManager.getConnectedProjectId());
    if (twitcherbotControl) {
        let mixplayControl = mixplayClient.state.getControl(id);
        if (mixplayControl) {
            mixplayControl.update(mapMixplayControl(twitcherbotControl));
        }
    }
});
...
frontendCommunicator.onAsync("controlAdded", async data => {
    ...
    let { sceneId } = data;
    let twitcherbotControl = data.newControl;

    let mixplayControl = mixplayClient.state.getControl(twitcherbotControl.id);
    ...
    if (scene) {
        await scene.createControl(mapMixplayControl(twitcherbotControl));
    }
    return true;
});
