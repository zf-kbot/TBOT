...
async function verifyAndGetUsers(twitchUsername, mixerUsername) {
    ...
    if (twitchUser == null) {
        throw new Error(`Twitch user '${twitchUsername}' not found in Twitchbot data`);
    }

    const mixerUser = await userDatabase.getMixerUserByUsername(mixerUsername);
    if (mixerUser == null) {
        throw new Error(`Mixer user '${mixerUsername}' not found in Twitchbot data`);
    }
...
async function addLinkRequest(twitchUsername, mixerUsername) {
    ...
    // throws an error if either the twitch or mixer user doesnt exist in twitcherbot
