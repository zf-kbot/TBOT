twitcherbotOverlay = new EventEmitter();
...
function mixerSocketConnect() {
	if ("WebSocket" in window) {
		...
		// When we get a message from the Twitchbot GUI...
		ws.onmessage = function (evt) {
			...
			twitcherbotOverlay.emit(event, data.meta);
		};
		...
		ws.onerror = function () {
			notification('open', `Connecting to Twitchbot... (${port}).`);
		}
