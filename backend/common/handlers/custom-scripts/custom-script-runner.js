...
async function runScript(effect, trigger) {
    ...
    if (manifest.startupOnly && !(trigger.type === "startup_script" || ((trigger.type === "event" || trigger.type === "manual") &&
        trigger.metadata.event.id === "twitcherbot-started"))) {
        renderWindow.webContents.send("error", `Could not run startup-only script "${manifest.name}" as it was executed outside of Twitchbot startup (Settings > Advanced > Startup Scripts)`);
        return;
    }
