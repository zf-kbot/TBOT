...
exports.run = async () => {
    ...
    for (let mixPlayFile of mixPlayProjectFiles) {
        ...
        devlabImporter.mapDevLabScenesToTwitchbotProject(data.mixer, newProject);

        let twitcherbotData = data.twitcherbot;
        if (twitcherbotData != null) {

            // import cooldown groups if they exist
            if (twitcherbotData.cooldownGroups != null) {
                let cooldownGroups = mapCooldownGroups(twitcherbotData.cooldownGroups);
                newProject.cooldownGroups = cooldownGroups;
            }

            if (twitcherbotData.controls != null) {
                let v4Controls = Object.values(twitcherbotData.controls);
