        ...
        .component("createSetupModal", {
            template: `
                ...
                <div class="modal-body">
                    ...
                    <h3>Components</h3>   
                    <p class="muted">Select the various components that you want to include in this Twitchbot Setup.</p>
                    ...
                    <h3>Options</h3>
                    <div>
                        <label class="control-fb control--checkbox" style="margin-bottom: 0px; font-size: 13px;opacity.0.9;"> Require User To Select A Currency <tooltip text="'Require the user to select one of THEIR currencies before importing. Twitchbot will update all Currency Effects, Variables, and Restrictions in the included components to use the selected currency. This is great for Chat games.'"></tooltip>
                            <input type="checkbox" ng-model="$ctrl.setup.requireCurrency">
                            <div class="control__indicator"></div>
                        </label>
                    </div>

                    <h3>Import Questions <tooltip text="'Users must supply answers to these questions before importing this Setup. Twitchbot will automatically replace all instances of the given token with the user\\'s answer.'"/></h3>
                    <div>
                        <div>
            ...
            controller: function (commandsService, countersService, currencyService,
                ...
                $ctrl.save = () => {
                    ...
                    const saveDialogOptions = {
                        ...
                        filters: [
                            { name: "Twitchbot Setup Files", extensions: ['twitcherbotsetup'] }
                        ],
                ...
                $ctrl.loadPreviousSetup = () => {
                    $q
                        .when(backendCommunicator.fireEventAsync("open-file-browser", {
                            options: {
                                filters: [{ name: 'Twitchbot Setups', extensions: ['twitcherbotsetup'] }]
                            }
                        }))
                        .then(response => {
                ...
                $ctrl.openComponentListModal = (label, allComponents, selectedIds, closeCallback) => {
                    utilityService.showModal({
                        component: "twitcherbotComponentListModal",
                        size: 'sm',
                        resolveObj: {
                            label: () => label,
