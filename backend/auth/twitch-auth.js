...
const gaManager = require("../common/ga-manager");
...
const STREAMER_ACCOUNT_PROVIDER = {
    ...
    scopes: [
        ...
        'channel:read:polls', //新增查看投票功能
        'channel:manage:polls', //新增管理投票功能
        'channel_commercial',
        'channel_editor',
        'channel_read',
...
authManager.on("auth-success", async authData => {
    ...
    if (providerId === STREAMER_ACCOUNT_PROVIDER_ID || providerId === BOT_ACCOUNT_PROVIDER_ID) {
        ...
        frontendCommunicator.send('auth-success');
        gaManager.sendEvent('twitch_user_info', 'u_id&u_name', `${userData.id}_${userData.display_name}`);
    }
});